    ...
    if request.method == 'POST':
        ...
        if uploaded_file:
            try:
                ...
                # 머신러닝 예측: 다음 달 총액 예측
                X = monthly_sum_display.index.values.reshape(-1, 1)  # 월(1,2,...)
                y = monthly_sum_display['총액'].values.reshape(-1, 1)
                ...
                # 다음 달 예측값 그래프에 추가
                plt.plot(next_month.flatten(), [pred], marker='o', color='red', label='예상 총액')
                plt.text(next_month.flatten()[0], pred, f'{pred:.1f}만원 (예상)', fontsize=10, ha='center', va='bottom', color='red')

                plt.title('월간 지출 (자동 반영 + 예측)', fontsize=16)
                plt.xlabel('월별', fontsize=14)
                plt.ylabel('총액 (만원)', fontsize=14)
                plt.xticks(list(monthly_sum_display.index) + [next_month.flatten()[0]], fontsize=12)
