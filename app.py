        ...
        if uploaded_file:
            try:
                ...
                for category in monthly_sum_display.columns:
                    ...
                    for x, y in zip(monthly_sum_display.index, monthly_sum_display[category]):
                        plt.text(x, y, f'{y:.1f}만원', fontsize=10, ha='center', va='bottom')

                plt.title('월간 지출 (자동 반영)', fontsize=16)
                plt.xlabel('월별', fontsize=14)
                plt.ylabel('총액 (만원)', fontsize=14)
                plt.xticks(monthly_sum_display.index, fontsize=12)
